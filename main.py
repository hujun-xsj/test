...
permissions_dict = {
    ...
    "ANDROID.PERMISSION.CHANGE_CONFIGURATION": "允许W修改当前设置,比如语言种类，屏幕方向",
}
...
def api_conclusion(dir):
    premission = {}
    for filename in os.listdir(dir):
        list = get_list(os.path.join(dir, filename))
        for api in list:
            if api not in premission:
                premission[api] = 1
            else:
                premission[api] = premission[api] + 1
    f = sorted(premission.items(), key=lambda x: x[1], reverse=True)
    for i in f:
        print(i)
if __name__ == '__main__':
    # get_json('D://google//muhan.apk')
    # get_permissions(get_list('D://google//muhan.apk'))
    api_conclusion('E://app')
